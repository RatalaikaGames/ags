...
//---------------------------------------------
//MBG - NEW TEXT ENCODE HANDLING
//we should ingest nothing but UTF-8; everything else is obsolete
...
void alfont_textout_aa_ex(BITMAP *bmp, ALFONT_FONT *f, const char *s, int x, int y, int color, int backg) {
  ...
  if (f->autofix==TRUE) {
	if (f->type==2) {
		...
		memset(lpszW, 0, nLen);
		ret = musl_mbstowcs((wchar_t *)lpszW, s_pointer, nLen);
  ...
  if (f->type==1) {
	...
	memset(lpszW, 0, nLen);
	lpszW_pointer = lpszW;
    musl_mbstowcs((wchar_t *)lpszW, s_pointer, nLen);
	#endif
  }
  else {
    #ifdef ALFONT_LINUX
...
int alfont_text_length(ALFONT_FONT *f, const char *str) {
  ...
  if (f->autofix==TRUE) {
	if (f->type==2) {
		...
		lpszW = (char *)malloc(nLen*sizeof(wchar_t));
		memset(lpszW, 0, nLen);
		ret = musl_mbstowcs((wchar_t *)lpszW, str_pointer, nLen);
  ...
  if (f->type==1) {
	...
	memset(lpszW, 0, nLen);
	lpszW_pointer = lpszW;
    musl_mbstowcs((wchar_t *)lpszW, str_pointer, nLen);
	#endif
  }
  else {
    #ifdef ALFONT_LINUX
...
int alfont_text_count(ALFONT_FONT *f, const char *str) {
  ...
  if (f->type==1) {
	...
	memset(lpszW, 0, nLen);
	lpszW_pointer = lpszW;
	string_count = musl_mbstowcs((wchar_t *)lpszW, str_pointer, nLen);
	#endif
  }
  else {
    #ifdef ALFONT_LINUX
